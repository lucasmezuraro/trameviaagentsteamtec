...
import { validatePlan, planWaves } from './policy.mjs';
...
try {
  ...
  if (extra.length || !['validate','plan','simulate'].includes(command) ||
      (command !== 'simulate' && !path) || (command === 'simulate' && path))
    throw new Error('uso: node src/cli.mjs validate|plan <plano.json> OU node src/cli.mjs simulate');
  ...
  const result = command === 'validate' ? validatePlan(plan, policy, roles)
    : command === 'plan' ? planWaves(plan, policy, roles) : simulate(plan, policy, roles);
